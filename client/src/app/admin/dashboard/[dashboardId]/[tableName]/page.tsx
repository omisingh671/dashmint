...
import { Download, Search, ChevronUp, ChevronDown, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function DynamicTableView() {
  ...
  // Reset page/sorting if table changes
  useEffect(() => {
    setPage(1);
    setSearch('');
    setDebouncedSearch('');
  ...
  // Fetch Dashboard Info to verify permissions and get display settings
  ...
  const { data: queryData, isLoading: ql, isError, error, refetch } = useQuery<any>({
    queryKey: ['table-records', dashboardId, tableName, page, debouncedSearch, sortBy, sortOrder],
    queryFn: () =>
      api
        .get(`/query/${dashboardId}/${tableName}`, {
          params: {
            page,
            limit,
  ...
  const isSuperAdmin = dashboard?.assignments?.length === 0; // Super admin has no assignment rows (sees all)
  ...
  const currentModel = dashboard?.models?.find((m: any) => m.name === tableName);
  const friendlyTableName = formatDisplayName(currentModel?.displayName || tableName);
  ...
  const handleExportCSV = () => {
    ...
    const url = new URL(`http://localhost:5000/api/query/${dashboardId}/${tableName}/export`);
    ...
    return (
      <div className="p-8 flex-1 flex flex-col justify-center items-center">
        <div className="max-w-md bg-red-50 border border-red-200 p-6 rounded-2xl text-center space-y-4">
          <AlertCircle className="h-10 w-10 text-red-605 mx-auto" />
          <h2 className="text-lg font-bold text-red-800">Database Query Failure</h2>
          <p className="text-red-750 text-sm leading-relaxed">
            {(error as any)?.response?.data?.error || 'Could not query data from the database. Verify connection parameters or column configurations.'}
          </p>
          <button
            onClick={() => refetch()}
  ...
  return (
    <div className="p-6 lg:p-10 space-y-6 flex-1 flex flex-col min-h-0 min-w-0 w-full max-w-full relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 select-none relative z-10">
        <div>
          <h1 className="text-2xl font-black text-text-main tracking-tight">{friendlyTableName}</h1>
          <p className="text-xs text-text-muted font-mono mt-1 flex items-center gap-1.5">
            <span className="h-1.5 w-1.5 rounded-full bg-indigo-600"></span>
            Physical Name: {tableName}
          </p>
        </div>
              ...
              {rows.length > 0 ? (
                ...
                <tr>
                  <td colSpan={columns.length || 1} className="p-16 text-center text-text-muted text-sm font-medium">
                    {loading ? 'Fetching table records...' : 'No records found matching query filter.'}
                  </td>
                </tr>
              )}
            </tbody>
